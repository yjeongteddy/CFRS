...
cat > job_01.sh << EOF
#!/bin/bash
tgt_dir="/home/user_006/01_WORK/2025/NPP/05_DATA/processed/${TGT_NPP}/${TGT_TC}/14_RESETUP/$subdir/${INTENSITY}"
if [ -d "\$tgt_dir" ]; then
        cd "\$tgt_dir"
else
        mkdir -p "\$tgt_dir"
...
ln -sf 02_SCRIPT/INPUT_ReSU.csh .
...
ReGetDepthSetup_v2('${TGT_TC}', '${TGT_NPP}', '${TGT_SL}');\
ReGetWindSetup('${TGT_TC}', '${TGT_NPP}', '${TGT_SL}'); \
RePrepWaveSetup_v2('${TGT_TC}', '${TGT_NPP}', '${TGT_SL}'); exit;"
EOF
