function PostReSetup(tgt_tc, tgt_NPP, intensity, tc_int, rpath)
...
wpath = fullfile(dpath, tgt_tc, '14_RESETUP', subdir, intensity);
...
% p = pcolor(xMatOrg, yMatOrg, setup); set(p, 'EdgeColor', 'None');
...
c = colorbar(); caxis([0 1]);c.Ruler.TickLabelFormat = '%.1f';
colormap(jet);
...
xlim([min(xMatItp(:)) max(xMatItp(:))]);
ylim([min(yMatItp(:)) max(yMatItp(:))]);
set(gca,'FontSize',25);        
set(gca,'Box','on');
set(gca,'LineWidth',3);        
set(gcf,'Color','w');
...
text(x_s+0.001,y_s-0.0110,['Unit' ' : ' 'm'],'VerticalAlignment','top','FontSize',18);
