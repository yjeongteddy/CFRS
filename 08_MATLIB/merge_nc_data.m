function merge_nc_data(tg_tc, tg_NPP, opath, rpath, ppath, spath)
...
if nargin < 1, tg_tc  = '2009_MAYSAK'; end
...
%% Target year
from_ncep = extractYear(tg_tc) < 2004;
...
gsm_plevel = squeeze(min(min(plevel_gsm,[],1),[],2))' / 100;
% SIZE_GP = size(plevel_gsm);
% gsm_plevel = zeros(1, SIZE_GP(3));
% for p_id = 1:SIZE_GP(3)
%     gsm_plevel(p_id) = min(min(plevel_gsm(:,:,p_id))) ./ 100;
% end
...
parfor met_id = 1:length(gsmList)
    ...
    count = 1;
    % Loop over pressure levels in ETC_DATA
    for i_level = 1:length(plevel_p)
        temp_p = plevel_p(i_level);
        find_id = find(temp_p == gsm_plevel);
        if count == 1
        ...
        if numel(find_id) == 2
            ...
            intp_tt = griddata(double(p_lon_mat), double(p_lat_mat), NC_ph_tt(:,:,i_level)', lon_gsm, lat_gsm);
            intp_tt(isnan(intp_tt)) = 0;
            intp_ght = griddata(double(p_lon_mat), double(p_lat_mat), NC_ph_ght(:,:,i_level)', lon_gsm, lat_gsm);
            intp_ght(isnan(intp_ght)) = 0;
            intp_rh = griddata(double(p_lon_mat), double(p_lat_mat), NC_ph_rh(:,:,i_level)', lon_gsm, lat_gsm);
            intp_rh(isnan(intp_rh)) = 0;
            
            NS_GSM_u = B_u .* intp_u + (1 - B_u) .* GSM_u(:,:,find_id);
            NS_GSM_u(isnan(NS_GSM_u)) = 0;
            NS_GSM_v = B_v .* intp_v + (1 - B_v) .* GSM_v(:,:,find_id);
            NS_GSM_v(isnan(NS_GSM_v)) = 0;
            
            NS_GSM_rh = B .* intp_rh + (1 - B) .* GSM_rh(:,:,find_id);
            NS_GSM_rh(isnan(NS_GSM_rh)) = 0;
            
            NS_GSM_ght = B .* intp_ght + (1 - B) .* GSM_ght(:,:,find_id);
            NS_GSM_ght(isnan(NS_GSM_ght)) = 0;
            
            NS_GSM_tt = B .* intp_tt + (1 - B) .* GSM_tt(:,:,find_id);
            NS_GSM_tt(isnan(NS_GSM_tt)) = 0;
            
            GSM_u(:,:,find_id) = NS_GSM_u;
            GSM_v(:,:,find_id) = NS_GSM_v;
            GSM_rh(:,:,find_id) = NS_GSM_rh;
            GSM_tt(:,:,find_id) = NS_GSM_tt;
            GSM_ght(:,:,find_id) = NS_GSM_ght;
            
            continue;
        end
        
        % Otherwise, use the standard blending for this level
        intp_u = griddata(double(p_lon_mat), double(p_lat_mat), NC_ph_u(:,:,i_level)', lon_gu, lat_gu);
        intp_u(isnan(intp_u)) = 0;
        intp_v = griddata(double(p_lon_mat), double(p_lat_mat), NC_ph_v(:,:,i_level)', lon_gv, lat_gv);
    ...
    new_name = [spath 'nc_uv_merge_' gsmList(met_id).name];
